...
def test_basic(
    ...
    assert (
        ruiner.Template(f"<!-- (param){param_name} -->").rendered(params_one)
        == f"{param_value}"
    )
    assert (
        ruiner.Template(f"<!-- (param){param_name} -->\n").rendered(params_one)
        == f"{param_value}\n"
    )
...
def test_ref():
    ...
    assert (
        greeting.rendered(
            {"name": "username", "addition": {"action": "eat"}}, {"addition": addition}
        )
        == "Hello, username!\nNice to eat you!\n"
    )
    result = greeting.rendered(
        {
            "name": "username",
            "addition": [{"action": "meet"}, {"action": "eat"}, {"action": "split"}],
        },
        {"addition": addition},
    )
    assert result == (
...
def test_optional_ref():
    ...
    assert (
        ruiner.Template("<!-- (optional)(ref)test_optional_ref_1 -->").rendered(
            {}, templates
        )
        == ""
    )
