...
def test_nonexistent_param():
    assert (
        ruiner.Template("lalala<!-- (param)p -->lololo").rendered({}) == "lalalalololo"
    )
...
def test_nonexistent_ref_deep():
    with pytest.raises(KeyError):
        ruiner.Template("<!-- (ref)test_nonexistent_ref_2 -->").rendered(
            parameters={},
            templates={"test_nonexistent_ref_2": ruiner.Template("<!-- (ref)no -->")},
        )
...
def test_recursion():
    with pytest.raises(RecursionError):
        ruiner.Template("<!-- (ref)test_recursion -->").rendered(
            parameters={},
            templates={
                "test_recursion": ruiner.Template("<!-- (ref)test_recursion -->")
            },
        )
